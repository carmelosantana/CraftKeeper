...
| 9 | Umami optional, disabled by default, never blocks rendering/onboarding/requests/builds | `tests/Feature/Settings/OptionalIntegrationsTest.php` (never-configured/explicitly-disabled/incomplete/insecure-URL all assert zero `<script>` tag; the fully-configured case asserts exactly one deferred script tag and that it's never proxied through the backend); `tests/Feature/Settings/SettingsPagesTest.php` (renders and persists the analytics settings page); `tests/e2e/settings-and-integrations.spec.ts`'s disabled-by-default assertion; "never blocks builds" is structural — neither `composer.json` nor `package.json` has ever gained an analytics SDK dependency, so no build step can depend on Umami being reachable |
...
| 14 | All config writes: path containment, optimistic concurrency, validation, snapshot, atomic replacement, audit event | `tests/Unit/Filesystem/MinecraftPathTest.php` (containment); `tests/Feature/Config/ConfigConflictTest.php` (optimistic concurrency at the service layer — a stale base hash throws `ConfigConflict` and never overwrites, including the TOCTOU case where the file changes after `propose()` but before execution); `tests/Feature/Http/ConfigControllerTest.php` and `tests/Feature/Api/V1/ConfigApiTest.php` (the same stale-hash case asserted as an HTTP **409** through the web editor and REST API respectively); `tests/Unit/Config/Formats/*Test.php` (validation); `tests/Integration/Filesystem/AtomicFileWriterTest.php` (snapshot + atomic rename); `tests/Feature/Config/ConfigChangeServiceTest.php` (one audit event per approved change) |
...
| 25 | Every user-visible operation has idle/pending/success/failure/degraded/retry behavior | `resources/js/features/operations/OperationProgress.tsx` + `App\Operations\OperationStatus` enum (`Proposed`/`Approved`/`Running`/`Succeeded`/`Failed`/`Rejected`/`RolledBack`) drives this uniformly; `tests/e2e/server-operations.spec.ts` (proposal/review/approval/rejection panels, "Approval required"/"Awaiting approval" states) and the rest of `tests/e2e/*` exercise pending/consequence/approval states; degraded states are covered per-integration (RCON, AI, catalog — see `docs/security/threat-model.md`'s trust-boundary table, each with its own isolated-degradation test) |
